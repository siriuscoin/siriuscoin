...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("SSC")
    updateDisplayUnit();
}
...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SSC unit input in lnReqAmount, we generate the URI with SSC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(SiriuscoinUnits::format(SiriuscoinUnits::SSC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
